...
var gLoading = contexto+"/loading.html";
...
function showPopWin(url, width, height, top, title, time, returnFunc) {
	gPopFrame.src = url;
	gReturnFunc = returnFunc;
	window.setTimeout("open('" + width + "', '" + height + "', '" + title + "', '" + top + "');", time);
}

function open(width, height, title, top) {
	gPopupIsShown = true;
	disableTabIndexes();
	gPopupMask.style.display = "block";
	gPopupContainer.style.display = "block";
	centerPopWin(width, height, top);
	...
	setPopTitleAndRewriteTargets(title);
}

var gi = 0;
function centerPopWin(width, height, top) {
 ...
 */
function setPopTitleAndRewriteTargets(title) {
